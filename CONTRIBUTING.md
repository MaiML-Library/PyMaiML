...
- **署名済みMaiMLファイルを保存する際は、XMLの再整形(pretty print)、
  ...
  処理を行わないこと。** 見た目には無害に見える整形であっても、署名時に
  計算された正準化結果と一致しなくなり、検証で不一致になり得ます。
  現状の`dumps()`/`dump()`は既存の`Signature`を常に出力から除外するため
  この問題には直面しませんが(上記の通り)、将来「署名済みファイルを
  そのまま保存・コピーする」機能や、署名対応モジュール
  (`pymaiml.signature`等)を追加する場合は、この制約を必ず守ってください。
