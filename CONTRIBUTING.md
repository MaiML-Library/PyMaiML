...
- **署名済みMaiMLファイルを保存する際は、XMLの再整形(pretty print)・
  ...
  属性の並び替え・空要素の記法(`<a></a>`⇄`<a/>`)の変更など、署名対象
  XMLの正準化(C14N)結果を変化させうる処理を一切行わないこと。**
  これらはXMLとして意味的に同一に見える変更であっても、署名時に計算
  された正準化結果とは一致しなくなり、検証時に不一致となり得ます
  (Gitの`autocrlf`設定やエディタの「保存時に改行コードを統一」機能が、
  意図せずこれを引き起こすことがあります)。現状の`dumps()`/`dump()`は
  既存の`Signature`を常に出力から除外するためこの問題には直面しません
  が(上記の通り)、将来「署名済みファイルをそのまま保存・コピーする」
  機能や、署名対応モジュール(`pymaiml.signature`等)を追加する場合は、
  この制約を必ず守ってください。署名済みファイルをGitリポジトリで
  管理する場合は、`.gitattributes`で該当ファイルを`-text`(改行コード
  正規化の対象外)に設定することも検討してください。
